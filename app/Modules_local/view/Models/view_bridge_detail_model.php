...
class view_bridge_detail_model extends Model
{
	...
	* $selAgency | agency 
	*/
	public function getoldcbridges($syear,$eyear,$x,$selAgency = '',$selProvince = '', $permittedDist = '') { 
    ...
    */
    public function getbridges($dataStart, $dateEnd, $dd = '', $ctype = ENUM_NEW_CONSTRUCTION, $resultType = 'array', $municipality = '', $orderby= '', $permittedDist = '') {

        //with bridge completion fiscal year
        ...
        //using project fiscal year
        // $sql = "select * from `view_bridge_major_dist_completed` where `bri03project_fiscal_year` >= '$dataStart' AND `bri03project_fiscal_year` <= '$dateEnd' AND `bri05bridge_complete_check` = 1 AND `bri05bridge_completion_fiscalyear_check` = 1 AND `bri03construction_type` = '$ctype'";
    ...
    public function getbridgesbydate($dataStart, $dateEnd, $dd = '', $ctype = ENUM_NEW_CONSTRUCTION, $resultType = 'array', $selAgency= '') {
        ...
        //restrict users 
        $arrPermittedDistList = $this->getPermittedUsers();
        $intUserType = (session()->get('type')) ? session()->get('type') : ENUM_GUEST;
        if ($intUserType == ENUM_REGIONAL_MANAGER || $intUserType == ENUM_REGIONAL_OPERATOR) {
          //comma seperated value
          if (count($arrPermittedDistList) > 0) {
    ...
    public function getbridgesbysup($dataStart, $dateEnd, $dd = '', $ctype = ENUM_NEW_CONSTRUCTION, $resultType = 'array', $selAgency = '') {
        // $sql = "select * from `view_bridge_major_dist_sup` where `bri03project_fiscal_year` >= '$dataStart' AND `bri03project_fiscal_year` <= '$dateEnd' AND `bri05bridge_complete_check` = 1 AND `bri05bridge_completion_fiscalyear_check` = 1 AND `bri03construction_type` = '$ctype'";
    ...
    public function getbridgesMunc($dataStart, $dateEnd, $dd = '', $ctype = ENUM_NEW_CONSTRUCTION, $resultType = 'array', $municipality = '') {
        // $sql = "select * from `view_bridge_major_dist_munc_completed` where `bri03project_fiscal_year` >= '$dataStart' AND `bri03project_fiscal_year` <= '$dateEnd' AND `bri05bridge_complete_check` = 1 AND `bri05bridge_completion_fiscalyear_check` = 1 AND `bri03construction_type` = '$ctype'";
    ...
    /*
    * get completed bridges by province
    */
    public function getbridgesbyProv($dataStart, $dateEnd, $dd = '', $ctype = ENUM_NEW_CONSTRUCTION, $resultType = 'array', $province = '', $orderby= '', $page = '') {

        //with bridge completion fiscal year
        $sql = "select a.* from `view_bridge_major_dist_completed` a ";
        //$sql .= $this->joinActualCost();
        $sql .= " where a.`bri05bridge_completion_fiscalyear` >= '$dataStart' AND a.`bri05bridge_completion_fiscalyear` <= '$dateEnd' AND a.`bri05bridge_complete_check` = 1 AND a.`bri05bridge_completion_fiscalyear_check` = 1 AND a.`bri03construction_type` = '$ctype'";
    ...
    public function getbridgesbyProvdate($dataStart, $dateEnd, $dd = '', $ctype = ENUM_NEW_CONSTRUCTION, $resultType = 'array', $province= '', $orderby= 'dist01state') {
        // /die($resultType);
        $sql = "select * from `view_bridge_major_dist_completed` where `bri05bridge_complete` >= '$dataStart' AND `bri05bridge_complete` <= '$dateEnd' AND `bri05bridge_complete_check` = 1 AND `bri05bridge_completion_fiscalyear_check` = 1 AND `bri03construction_type` = '$ctype'";
        if($dd != '') {
            if(is_array($dd)) {
    ...
    * Get actual cost
    */
    public function joinActualCost() 
    {
        // $this->view_bridge_actual_cost->
        //             whereIn('bri08bridge_no', $arrBridgeIdList)->
        //             asObject()->
        //             findAll();
        $sql = " LEFT JOIN `view_bridge_actual_cost` v ON a.bri08bridge_no = v.bri08bridge_no";
        return $sql;
    }
