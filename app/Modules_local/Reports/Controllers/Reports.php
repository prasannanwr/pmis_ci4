...
*/
    ...
    public function loadMore($page = '')
  {
    ...
    if ($dataStart <= $dateEnd)
    {
        if ($dataStart != 0 || $dateEnd != 0)
        {
            $arrCostCompList = $this->cost_components_model->asObject()->findAll();
            $arrPrintList = array();
            $arrCostList = array();

            if(trim($selProvince) != '') {
                $arrDevList = $this->district_name_model->where('dist01state',$selProvince)->findAll();
            } else {
                $arrDevList= $this->district_name_model->asObject()->findAll();
            }
