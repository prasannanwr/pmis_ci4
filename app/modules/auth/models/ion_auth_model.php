...
class Ion_auth_model extends CI_Model
{
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		log_query('Employee Created');
		//Add userinfo in access table
		/*$new_user['user_id'] = $id;
		$this->load->module('employee_access');
		$this->employee_access->_insert($new_user);
		*/
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', username, email, id, password, active, last_login, type, regional_office')
	...
	{
		$this->_ion_select = array('id', 'first_name', 'last_name');
		$this->_ion_where = array(array('active' => 1, 'id !=' => 1));
		$this->db->where("(first_name LIKE '%$like%' OR last_name LIKE '%$like%')", null, false);
	...
	{
		$this->_ion_select = array('id', 'first_name', 'last_name');
		$this->_ion_where = array(array('id !=' => '1'));
	...
	{
		$this->_ion_select = array('id', 'first_name', 'last_name');
		$this->_ion_where = array(array('id' => $user_id));
		return $this->users();
	}
	
	public function get_hierarchy($id)
	{
		$this->_ion_select = array('id', 'first_name', 'last_name');
		$this->_ion_where = array(array('manager_id' => $id));
		$this->_ion_order_by = 'first_name';
		$this->_ion_order = 'asc';
		return $this->users();
	}
	...
	{
		//$this->_ion_where = array(array('active' => 1));
		$this->_ion_where = array(array('active' => 1, 'id !=' => 1));
		$this->_ion_order_by = 'first_name';
		$this->_ion_order = 'asc';
		return $this->users();
	...
	public function user_fullname($id)
	{
		...
		//if no id was passed use the current users id
		//$id || $id = $this->session->userdata('user_id');
		$this->_ion_select = array('first_name', 'last_name', 'username');
		$this->limit(1);
		$this->where($this->tables['users'].'.id', $id);
	...
	**/
	...
	public function set_session($user)
	{
		...
		$session_data = array(
		    ...
		    'type'                 => $user->type, //everyone likes to overwrite id so we'll use user_id
		    'region'			   => $user->regional_office
		);
	...
	public function login_remembered_user()
	{
		...
		//get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', id, username, email, last_login, type')
	...
	**/
	...
	function get_employee_hierarchy($user_id)
	{
		$mysql_query = "SELECT emp.* FROM (
				SELECT id, first_name, last_name, 'manager' AS who, 0 AS 'level' FROM EMPLOYEE WHERE ID IN (SELECT manager_id FROM employee WHERE id = $user_id)
				UNION
				SELECT id, first_name, last_name, 'self' AS who, 1 AS 'level' FROM EMPLOYEE WHERE id = $user_id
				UNION
				SELECT id, first_name, last_name, 'employee' AS who, 2 AS 'level' FROM EMPLOYEE WHERE manager_id = $user_id
				) emp
				ORDER BY emp.level ASC, emp.first_name ASC, emp.last_name ASC";
