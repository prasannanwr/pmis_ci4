...
class Material_issue extends MX_Controller {
	...
	function index()
	{
        ...
        if  ($this->session->userdata('type') == ENUM_ADMINISTRATOR) {
            $data['arrDataList']= $this->model->getOverallList();
        } else {
    ...
    function create($emp_id = FALSE){
        ...
        if( $emp_id !== false){
            ...
            $objOldRec = $this->model->issueDetail($emp_id);
            $data['objOldRec'] = $objOldRec;
            $requistion_num = $objOldRec->requisition_num;
            $materialIssueinfo = $this->model->issueInfo($requistion_num);        
            $data['materialIssueinfo'] = $materialIssueinfo;
            //var_dump($data['objOldRec']);exit;
            $data['postURL'] .= '/'.$emp_id;
        }
...
//        $data['suppliers_list'] = $this->supplier_model->dbGetList();
		...
		else // passed validation proceed to post success logic
		{
            ...
            if($units <= $available_units) {
                ...
                //redirect(self::$fName, 'refresh');
                if( $emp_id !== false){
                    redirect('material_issue/create/'.$emp_id, 'refresh');
                }
                redirect('material_issue/create', 'refresh');
    ...
    function delete()
	{
        ...
        $this->model->where('id', $delete_id)->dbDelete();
        $this->stock_model->addStock($delete_id);
        $message = 'Selected Data Deleted.';
        log_query($message);
        set_message($message, 'success');
		redirect('material_issue');
	}

	function getRequisitionBridge() {
	    $requistion_num = $_GET['receipt_num'];
        $reqinfo = $this->requisition_model->requisitionInfoByRid($requistion_num);                
        echo json_encode(array("bridge" => $reqinfo->bridge_name, "bridge_num" => $reqinfo->bridge_num));
    }
    ...
    function getRequisitionInfo() {
        ...
        //print_r($str);exit;
        echo json_encode(array("issued" => $reqinfo->name, "bridge" => $reqinfo->bridge_name, "bridge_num" => $reqinfo->bridge_num, "issued_data" => $str));
    }
