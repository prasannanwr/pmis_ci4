...
class material_issue_model extends ET_BaseModel {
    ...
    */
    public function issueDetail($req_num) {
        ...
        $sql = "SELECT a.`requisition_num`,a.`issued_date`,b.`bridge_name`,b.`bridge_num`,c.`name` as org_name,d.`dist_name`,d.`dist_state` FROM `requisition` b LEFT JOIN `material_issue` a ON a.requisition_num=b.id  LEFT JOIN `organization` c ON b.issued_for=c.organization_id LEFT JOIN `district` d ON b.district=d.dist_id LEFT JOIN `material_receipt` e ON a.lot=e.id LEFT JOIN `material_type` f ON e.type=f.id WHERE b.id='$req_num'";        
        $query = $this->db->query($sql);
        return $query->row();
    }
    ...
    */
    public function getIssuedList($region) {
        ...
        $sql = "SELECT `id`,`req_id` FROM `requisition` WHERE `region` = '$region'";
        $query = $this->db->query($sql);
        return $query->result();
    ...
    */
    ...
    public function getIssueList($receipt_id) {
        $sql = "SELECT a.`id`,a.`units`,a.`issued_date`,b.`req_id` FROM `material_issue` a LEFT JOIN `requisition` b ON a.`requisition_num`=b.`id` LEFT JOIN `material_receipt` c ON b.`issued_for` = c.`id` WHERE a.`lot` = '$receipt_id'";
        $query = $this->db->query($sql);
        return $query->result();
    }
    ...
    */
    public function getIssueByOrganization($organization,$type) {

        $sql = "SELECT a.issued_date,a.lot,a.units,b.diameter,d.name as diametername,e.name as organization_name FROM material_issue a LEFT JOIN material_receipt b ON a.lot = b.id LEFT JOIN requisition c ON a.requisition_num = c.id LEFT JOIN diameter d ON b.diameter = d.id LEFT JOIN organization e ON c.issued_for = e.organization_id WHERE c.issued_for = '$organization' AND b.type = '$type' order by b.diameter ASC";
        $query = $this->db->query($sql);
        return $query->result();
    }
    ...
    public function totalUnitsByOrganization($organization='',$type='') {
     ...
     */
    public function totalUnitsOrganization($type='') {
     ...
     */
    public function totalUnitsDiameter($type='',$agency='') {

        $sql = "SELECT SUM(a.units) as totalunits,d.name as diametername FROM material_issue a LEFT JOIN material_receipt b ON a.lot = b.id LEFT JOIN diameter d ON b.diameter = d.id WHERE b.type = '$type'";
        if($agency != '') {
     ...
     */
    public function totalUnitsReelno($type='',$agency='') {

        $sql = "SELECT SUM(a.units) as totalunits,b.lot,d.name as diametername FROM material_issue a LEFT JOIN material_receipt b ON a.lot = b.id LEFT JOIN diameter d ON b.diameter = d.id WHERE b.type = '$type'";
        if($agency != '') {
            $sql .=" AND b.purchased_by = '$agency'";
        }
        $sql .=" GROUP BY lot";
    ...
    public function totalUnitsRegion($type='',$agency='',$region ='') {
        ...
        $sql = "SELECT SUM(a.units) as totalunits,b.units as total_receipt,d.name as diametername FROM material_issue a LEFT JOIN material_receipt b ON a.lot = b.id LEFT JOIN diameter d ON b.diameter = d.id WHERE b.type = '$type' AND b.region = '$region'";
        if($agency != '') {
            $sql .=" AND b.purchased_by = '$agency'";
        }
        $sql .=" GROUP BY b.region";
        $query = $this->db->query($sql);
        return $query->result();
    }
    ...
    public function totalOverallUnits($type='',$agency='',$region ='') {
        ...
        $sql = "SELECT SUM(a.units) as total_issue,SUM(b.units) as total_receipt,d.name as diametername FROM material_issue a LEFT JOIN material_receipt b ON a.lot = b.id LEFT JOIN diameter d ON b.diameter = d.id WHERE b.type = '$type' AND b.region = '$region'";
        if($agency != '') {
            $sql .=" AND b.purchased_by = '$agency'";
        }
