...
class Organization extends MX_Controller {
    ...
    function create($emp_id = FALSE){
        ...
        if( $emp_id !== false){
            $data['objOldRec'] = $this->model->where('organization_id',$emp_id)->dbGetRecord();
            $data['postURL'] .= '/'.$emp_id;
        }
		...
		else // passed validation proceed to post success logic
		{
			...
			{
    			set_message('Organization successfully created.', 'success');
    ...
    function delete()
	{
		...
		$this->model->where('organization_id', $delete_id)->dbDelete();
